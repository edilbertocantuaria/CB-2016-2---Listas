    ...
    while(fscanf(pFile, "%d %s %d %d %d %s %s %s", &c.id, c.nome, &c.dia, &c.mes, &c.ano, c.email, c.t_movel, c.t_fixo)!=EOF){
        ...
        for(x=0; x<13; x++){
            if(x<2)
                ...
                mudar[x]=c.t_movel[x-1];
        }

        fprintf(pmudar, "%d %s %d %d %d %s %s %s\n", c.id, c.nome, c.dia, c.mes, c.ano, c.email, mudar, c.t_fixo);
        contador++;
    }
    fclose(pmudar);
